...
def main(args, i=0):
    ...
    if args.dataset == 'correlated_Blackscholes':
        ...
        spec = ('mu={}_sigma={}_window_size={}'.format(data_params['data_params']['mu'], data_params['data_params']['vola_matrix'], sig_window_size))
        ...
        spec = ('mu={}_sigma={}_window_size={}'.format(data_params['data_params']['lambda_0'], data_params['data_params']['v0_sqrt'], sig_window_size))
...
def run(args, data_params, returns, stock_data, spec):
    ...
    if os.path.exists(f"{model_save_path}.zip"):
        ...
        model = TD3("MlpPolicy", vec_env, buffer_size=args.buffer_size, gamma=1, learning_rate=args.learning_rate, action_noise=action_noise, batch_size=args.batch_size, verbose=0, tensorboard_log="./logs/", train_freq=(args.train_freq, "step"))
