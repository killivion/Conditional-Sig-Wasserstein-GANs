...
class PortfolioEnv(gym.Env):
    def __init__(self, args, data_params, stock_returns, stock_data):
        ...
        feature_size = self.num_stocks-1 + len(self.mu) + len(self.vola_matrix.flatten())  # + len(self.mu) + len(self.vola_matrix.flatten())
    ...
    def _get_feature_map(self):
        ...
        feature_map = np.concatenate([self.stock_data[self.current_step], self.mu, self.vola_matrix.flatten()])  #np.concatenate([normalized_returns, self.mu, self.vola_matrix.flatten()])
        return feature_map
